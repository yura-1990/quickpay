    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        <form action="{{ route('roles.update', ['role' => $role->id]) }}" method="POST" >
            ...
            <div class="w-full">
                ...
                <x-text-input id="role-name" class="w-full" name="name" value="{{ $role->name }}" />
            </div>

            <div>
                @foreach($permissions as $permission)
                    <div class="flex items-center gap-4">
                        <input type="checkbox" @if($role->hasPermissionTo($permission->name)) checked @endif id="{{ $permission->name }}" name="permissions[]" value="{{ $permission->id }}"/>
                        <x-input-label value="{{ $permission->name }}" for="{{ $permission->name }}" class="text-lg font-samibold" />
                    </div>
                @endforeach
            </div>
